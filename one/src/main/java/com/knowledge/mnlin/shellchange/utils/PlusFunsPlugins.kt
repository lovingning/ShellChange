...
inline infix fun <T : Page> T.toast(msg: String?): T {
    Toast.makeText(this.pageContext.context, msg, Toast.LENGTH_SHORT).show()
    return this
}
 ...
 * 添加onClick防抖动监听
 */
fun <T : List<F>, F : View> T.dOnClick(action: F.(index: Int, item: F) -> Unit) {
...
fun <T : View> T.postV(action: (View) -> Unit) {
    this.post {
        action(this)
    }
}
...
inline fun <T : TextView> List<T>.doAfterTextChanged(crossinline callback: (allEditText: List<T>, item: T) -> Unit): List<T> {
    this.also { all ->
        all.forEach { item ->
